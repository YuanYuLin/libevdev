...
TARBALL_FILE="libevdev-1.5.9.tar.xz"
TARBALL_DIR="libevdev-1.5.9"
INSTALL_DIR="libevdev-bin"
pkg_path = ""
output_dir = ""
tarball_pkg = ""
tarball_dir = ""
...
def MAIN_BUILD(args):
    ...
    libevdev = "libevdev.so.2.1.21"
    ops.copyto(ops.path_join(install_tmp_dir, "usr/local/lib/" + libevdev), dst_lib_dir)
    ops.ln(dst_lib_dir, libevdev, "libevdev.so.2.1")
    ops.ln(dst_lib_dir, libevdev, "libevdev.so.2")
    ops.ln(dst_lib_dir, libevdev, "libevdev.so")

    ops.mkdir(tmp_include_dir)
    ops.copyto(ops.path_join(install_tmp_dir, "usr/local/include/libevdev-1.0/."), tmp_include_dir)
    return False
